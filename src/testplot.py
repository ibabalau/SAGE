...
def test_pred_sum(spdfa, rev_spdfa, X_test, Y_test, Y_test_as, strat):
    ...
    for test_trace in X_test:
        ...
        exectimes_dict[strat][len(test_trace)].append(time.time() - start_time)
...
def test_pdfa(pdfa, X_test, Y_test, Y_test_as):
    ...
    for tt in X_test:
        ...
        exectimes_dict['PDFA'][len(tt)].append(time.time() - start_time)
        y_pred.append(na)    
        skip_cnt += skip  
        # if get_attack_stage(pred[0]) == get_attack_stage(true_action):
        #     accuracy_per_as[as_to_sev[get_attack_stage(true_action)]] += 1
...
exectimes_dict = {strategy: {key:[] for key in range(2, max_len)} for strategy in ['PDFA']}

traces = parse_file(tr_file_name)
lens = [len(i) for i in traces]
#plot_values(None, lens, 'Length', 'Count', 'Length distribution of traces in CPTC-2018 dataset', 'plots/trace_lens.pdf')
...
for i in range(cnt):
    ...
    spdfa, rev_spdfa, symbol_to_state = create_structs(train_data, data, unique_sym)
...
for key,val in exectimes_dict.items():
    ...
    for k,sublist in val.items():
        if sublist != []:
            x_axis.append(k)
            y_values.append(np.sum(sublist)/len(sublist))
...
# Set the title and axis labels
ax.set_title('Execution time for different input sizes', fontsize=13, fontfamily='serif')
ax.set_xlabel('Input trace length, symbols', fontsize=12, fontfamily='serif')
...
fig.savefig("plots/exectime_pdfa.pdf", dpi=300)
