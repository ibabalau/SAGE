...
# To compute probability of a trace, multiply all states probabilities in that trace
...
trace_file = dir_path + '/pred_traces/trace_all.txt'
traces = parse_file(trace_file)


# 1. Split full trace file into train and test and then learn a model with the train data
trainlen, testlen = round(len(traces)*0.8), round(len(traces)*0.2)
print(len(traces), trainlen, testlen)
#choosing random items from a list to get train/test data split
...
for st in lines:
    f.write(st)
f.close()
full_model_name = trace_file_location + trace_file_name
...
# Learn S-PDFA
flexfringe(trace_file_location + trace_file_name, ini=path_to_ini, symbol_count='2', state_count='4')

os.system('dot -Tpng ' + full_model_name + '.ff.final.dot -o ' + output_path + '/' + trace_file_name + '.png')


print('------ !! Special: Fixing syntax error in main model and sink files  ---------')
print('--- Sinks')
with open(full_model_name + '.ff.finalsinks.json', 'r') as file:
    filedata = file.read()
...
if extracommas is not None:
    ...
    with open(full_model_name + '.ff.finalsinks.json', 'w') as file:
        file.write(filedata)

print('--- Main')  
with open(full_model_name + '.ff.final.json', 'r') as file:
    filedata = file.read()
stripped = re.sub('[\s+]', '', filedata)
extracommas = re.search('(}(,+)\]}$)', stripped)
if extracommas is not None:
    ...
    print(extracommas.group(0), c)
    filedata = ''.join(filedata.rsplit(',', c))
    with open(full_model_name + '.ff.final.json', 'w') as file:
        file.write(filedata)
...
# 2. Compute probabilities of nodes/states and edges/symbols on training data (double check!)
...
unique_sym.update(set([item for item in edges]))
...
print(rev_spdfa['7'])
print(end_symb_cnt[spdfa['7']['symbol']])
    ...
    while not q.empty():
        ...
        if current_symb == rev_spdfa[node]['symbol']:
            for item in rev_spdfa[node]['transitions']:
                #print('adding node', item['target'], 'new pr list', pr_list + [item['prob']], 'new trace', tr[1:])
...
succ_cnt = 0
tp_cnt = 0
for test_trace in test_data_uniq:
    test_trace.reverse()
    ...
    if test_trace == ['vulnD|http', 'dManip|http']:
        print(test_trace[0], start_nodes)
        print(spdfa['179'])
        print(rev_spdfa['179'])
...
test_tr_len = len(test_data_uniq)
